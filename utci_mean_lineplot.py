...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 12:14:21 2024
...
# Load the dataset
ds = xr.open_dataset(r'/home/lab/Desktop/Narayanswamy/heat_stress/heat/utci_daily.nc')
...
# Calculate yearly mean UTCI over the specified time range
utci_yearly_mean = utci.sel(time=slice('1990-03-01', '2020-06-30')).mean(dim=('lat', 'lon')).groupby('time.year').mean()

# Calculate the trends
#trend1 = t2m_time1 - t2m_time4
#trend2 = t2m_time2 - t2m_time4
#trend3 = t2m_time3 - t2m_time4
trend33 = utci_time3 - utci_time2
trend22 = utci_time2 - utci_time1
a = trend22 + trend33
...
time_hours = time.year + time.minute / 60.0
time_reversed = time_hours[::-1]
utci4_time_hours = time[:len(utci44)]

tt = np.arange(1990, 2021, 1)
levels = np.arange(23, 35.1, 1)
# Create a gridspec
#fig = plt.figure(figsize=(16, 10))
fig = plt.figure(figsize=(26, 12))
gs = gridspec.GridSpec(2, 3, width_ratios=[4, 4, 9])
...
pcm1 = ax1.contourf(lons, lats, utci_time4_masked, transform=ccrs.PlateCarree(),  levels = levels, cmap='jet', extend='both')
#ax1.add_feature(india_feature)

#ax1.coastlines()
ax1.set_title('Mean UTCI (1990-2020)', fontweight='bold')
ax1.set_ylabel('Latitude', fontweight='bold', fontsize=18)
#ax1.set_xlabel('Longitude')
ax1.set_xticks(np.arange(60, 100.1, 5))
ax1.set_xticklabels([])

ax1.set_yticks(np.arange(5, 37.6, 5), crs=ccrs.PlateCarree())
# ax1.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:.0f}°E'.format(x)), fontweight='bold')
...
pcm2 = ax2.contourf(lons, lats, utci_time1_masked, transform=ccrs.PlateCarree(), levels = levels, cmap='jet', extend='both')

#ax2.coastlines()
#ax2.set_ylabel('Latitude')
ax2.set_xticks(np.arange(60, 100.1, 5))
ax2.set_yticks(np.arange(5, 40, 5))
ax2.set_xticklabels([])
ax2.set_yticklabels([])

ax2.set_title('Mean UTCI (1990-2000)', fontweight='bold')
##x2.set_xlabel('Longitude')
# ax2.set_xticks(np.arange(65.7, 97.25, 5), crs=ccrs.PlateCarree(), fontweight='bold')
# ax2.set_yticks(np.arange(5, 37.6, 5), crs=ccrs.PlateCarree(), fontweight='bold')
# ax2.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:.0f}°E'.format(x)), fontweight='bold')
# ax2.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: '{:.0f}°N'.format(y)), fontweight='bold')
...
pcm3 = ax3.contourf(lons, lats, utci_time2_masked, transform=ccrs.PlateCarree(),  levels = levels, cmap='jet', extend='both')

#ax3.coastlines()
ax3.set_title('Mean UTCI (2001-2010)', fontweight='bold')
ax3.set_ylabel('Latitude', fontweight='bold', fontsize=18)
ax3.set_xlabel('Longitude', fontweight='bold', fontsize=18)
ax3.set_xticks(np.arange(60, 100.1, 5), crs=ccrs.PlateCarree())
ax3.set_yticks(np.arange(5, 40.1, 5), crs=ccrs.PlateCarree())
# ax3.set_xticklabels(ax3.get_xticks(), rotation=90, ha='center')
ax3.set_xticklabels(ax3.get_xticks(), rotation=45, ha='center')
ax3.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:.0f}°E'.format(x)))
ax3.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: '{:.0f}°N'.format(y)))
...
ax4.add_feature(shp)
#ax3.add_feature(states_feature)
pcm4 = ax4.contourf(lons, lats, utci_time3_masked, transform=ccrs.PlateCarree(),  levels = levels, cmap='jet', extend='both')
#ax4.coastlines()
ax4.set_title('Mean UTCI (2011-2020)', fontweight='bold')
#ax4.set_ylabel('Latitude')
ax4.set_xlabel('Longitude', fontweight='bold', fontsize=18)

ax4.set_xticks(np.arange(60, 100.5, 5), crs=ccrs.PlateCarree())
ax4.set_xticklabels(ax4.get_xticks(), rotation=45, ha='center')
ax4.set_yticks(np.arange(5, 40, 5))
ax4.set_yticklabels([])
# ax4.set_yticks(np.arange(5, 37.6, 5), crs=ccrs.PlateCarree(), fontweight='bold')
ax4.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: '{:.0f}°E'.format(x)))
# ax4.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: '{:.0f}°N'.format(y)), fontweight='bold')
...
#ax5.set_yl, fontsize=12abel('Latitude')
...
# ax6.plot(tt, utci_yearly_mean, label='1990-2020')
ax6.set_title('Climatological UTCI', fontweight='bold')
ax6.set_xlabel('Time', fontweight='bold', fontsize=18)
# ax6.set_xticklabels(ax6.get_xticks(), rotation=90, ha='center')
# Rotate the x-axis labels to a slight angle
ax6.tick_params(axis='x', labelrotation=45)  # Set the rotation to 45 degrees (or adjust as needed)

ax6.set_ylabel('Mean UTCI', fontweight='bold', fontsize=16)
...
cbar1.set_label('Mean UTCI (°C)', fontweight='bold', fontsize=18)
cbar1_ax = cbar1.ax
fig.subplots_adjust(bottom=0.00015)  # Adjust the bottom margin to create space for the colorbar
cbar1_ax.set_position([0.152, 0.255, 0.3, 0.2])  # (L-R, T-B, CBAR width, cbar length   Adjust the po,sition and size of the colorbar
